#include "sfMeshStandInManager.h"
...
sfMeshStandInManager::sfMeshStandInManager()
{
    m_onGetAssetPropertyHandle = sfPropertyUtil::OnGetAssetProperty().AddRaw(this, &sfMeshStandInManager::SendBounds);
    m_materialPtr = LoadObject<UMaterialInterface>(nullptr, TEXT("/SceneFusion/StandIns/QuestionMaterial"));
}

sfMeshStandInManager::~sfMeshStandInManager()
{
    sfPropertyUtil::OnGetAssetProperty().Remove(m_onGetAssetPropertyHandle);
}

void sfMeshStandInManager::CleanUp()
{
    m_meshBoundsPtr = nullptr;
}

void sfMeshStandInManager::OnCreate(sfObject::SPtr objPtr, int childIndex)
{
    m_meshBoundsPtr = objPtr->Property()->AsDict();
}

void sfMeshStandInManager::Generate(const FString& path, UObject* uobjPtr)
{
    ...
    meshPtr->SourceModels.Emplace();
    FStaticMeshSourceModel& model = meshPtr->SourceModels[0];
...
void sfMeshStandInManager::SendBounds(UObject* uobjPtr)
{
    UStaticMesh* meshPtr = Cast<UStaticMesh>(uobjPtr);
    if (meshPtr == nullptr)
    {
...
FBox sfMeshStandInManager::GetBounds(const sfName& path)
{
...
sfName sfMeshStandInManager::SanitizePath(FString path)
{
