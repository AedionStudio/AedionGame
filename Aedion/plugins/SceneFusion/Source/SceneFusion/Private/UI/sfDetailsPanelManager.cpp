    ...
    m_propertyEditorModule { propertyEditorModule }
{
    ...
    FEditorModeRegistry::Get().RegisterMode<FSceneFusionEdMode>(
        ...
        if (tabManagerPtr.IsValid())
        {
            ...
            if (tabPtr.IsValid())
            {
                TSharedPtr<SSCSEditor> editorPtr = StaticCastSharedPtr<SSCSEditor>(
                    sfUtils::FindWidget(tabPtr->GetContent(), "SSCSEditor"));
...
{
    bool editable = true;
    if (m_sessionPtr != nullptr && SceneFusion::ActorManager.IsValid() && objects.Num() > 0)
    {
...
TSet<AActor*> sfDetailsPanelManager::GetSelectedActors()
{
    ...
    for (TSharedPtr<IDetailsView> detailsViewPtr : m_detailsViews)
    {
        if (!detailsViewPtr.IsValid() || !detailsViewPtr->IsLocked())
        {
            continue;
        }
